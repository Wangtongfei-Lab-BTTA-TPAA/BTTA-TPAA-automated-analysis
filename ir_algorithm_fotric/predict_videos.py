...
def get_frame_mask(ps, h, w, maskimg_saved_path):
    '''
    构造多边形mask，ps中的点必须按照顺序排列，不一定非得四个点，更多的点也支持
    :param ps:
    :return:
    '''
    mask = cv2.fillConvexPoly(np.zeros([h, w], dtype=np.uint8), np.array(ps), 255)
    cv2.imwrite(str(maskimg_saved_path), mask)
    return mask != 0


def create_tracking_dataset(
        config, videos, track_method, videotype="", shuffle=1, trainingsetindex=0, gputouse=None, save_as_csv=False,
        destfolder=None, batchsize=None, cropping=None, TFGPUinference=True, dynamic=(False, 0.5, 10),
        modelprefix="", robust_nframes=False, n_triplets=1000,
):
    try:
...
def analyze_videos(
        config, videos, videotype="", shuffle=1, trainingsetindex=0, gputouse=None, save_as_csv=False,
        in_random_order=True, destfolder=None, batchsize=None, cropping=None, TFGPUinference=True,
        dynamic=(False, 0.5, 10), modelprefix="", robust_nframes=False, allow_growth=False, use_shelve=False,
        auto_track=True, n_tracks=None, calibrate=False, identity_only=False,
        use_openvino="CPU" if is_openvino_available else None,
        Mask_polygon_vertexs=None  # 点集list or 点集str or 'None'str
):
    ...
    if isinstance(Mask_polygon_vertexs, str):
        if Mask_polygon_vertexs == 'None':
            Mask_polygon_vertexs = None
        else:
            _ = list(map(int, Mask_polygon_vertexs.replace('[', '').replace(']', '').split(',')))
            xs = _[::2]
            ys = _[1::2]
            Mask_polygon_vertexs = list(zip(xs, ys))
    ...
    # Name for scorer:
    DLCscorer, DLCscorerlegacy = auxiliaryfunctions.get_scorer_name(
        cfg, shuffle, trainFraction, trainingsiterations=trainingsiterations, modelprefix=modelprefix,
    )
    if dlc_cfg["num_outputs"] > 1:
        if TFGPUinference:
            print(
    ...
    if len(Videos) > 0:
        if "multi-animal" in dlc_cfg["dataset_type"]:
            ...
            for video in Videos:
                AnalyzeMultiAnimalVideo(
                    video, DLCscorer, trainFraction, cfg, dlc_cfg, sess, inputs, outputs, destfolder,
                    robust_nframes=robust_nframes, use_shelve=use_shelve,
                )
                if auto_track:  # tracker type is taken from default in cfg
                    convert_detections2tracklets(
                        config, [video], videotype, shuffle, trainingsetindex, destfolder=destfolder,
                        modelprefix=modelprefix, calibrate=calibrate, identity_only=identity_only,
                    )
                    stitch_tracklets(
                        config, [video], videotype, shuffle, trainingsetindex, destfolder=destfolder,
                        n_tracks=n_tracks, modelprefix=modelprefix, save_as_csv=save_as_csv,
                    )
        else:
            for video in Videos:
                DLCscorer = AnalyzeVideo(
                    video, DLCscorer, DLCscorerlegacy, trainFraction, cfg, dlc_cfg, sess, inputs, outputs,
                    pdindex, save_as_csv, destfolder, TFGPUinference, dynamic, use_openvino, Mask_polygon_vertexs
                )
...
def GetPoseF_GTF(cfg, dlc_cfg, sess, inputs, outputs, cap, nframes, batchsize,
                 maskimg_saved_path, Mask_polygon_vertexs):
    """Batchwise prediction of pose"""
    PredictedData = np.zeros((nframes, 3 * len(dlc_cfg["all_joints_names"])))
    batch_ind = 0  # keeps track of which image within a batch should be written to
    batch_num = 0  # keeps track of which batch you are at
    ...
    if Mask_polygon_vertexs is None:
        mask = np.ones([h, w], bool)
    else:
        mask = get_frame_mask(Mask_polygon_vertexs, h, w, maskimg_saved_path)
        ret, frame = cap.read()
        ps = Mask_polygon_vertexs + [Mask_polygon_vertexs[0]]
        for i in range(len(Mask_polygon_vertexs)):
            cv2.line(frame, (ps[i][0], ps[i][1]), (ps[i + 1][0], ps[i + 1][1]), (255, 255, 255), 2, cv2.LINE_AA)
        cv2.imwrite(maskimg_saved_path[:-8] + 'draw_polygon.jpg', frame)
...
def AnalyzeVideo(
        video, DLCscorer, DLCscorerlegacy, trainFraction, cfg, dlc_cfg, sess, inputs, outputs, pdindex, save_as_csv,
        destfolder=None, TFGPUinference=True, dynamic=(False, 0.5, 10),
        use_openvino="CPU" if is_openvino_available else None,
        Mask_polygon_vertexs=None
):
    ...
    maskimg_saved_path = str(Path(video).parent / f'{vname}_frame_mask.jpg')
    ...
    try:
        ...
        if dynamic_analysis_state:
            ...
            if int(dlc_cfg["batch_size"]) > 1:
                args = (
                    cfg, dlc_cfg, sess, inputs, outputs, cap, nframes, int(dlc_cfg["batch_size"]))
                if use_openvino:
                    PredictedData, nframes = GetPoseF_OV(*args)
                elif TFGPUinference:
                    args = list(args) + [maskimg_saved_path, Mask_polygon_vertexs]
        ...
        auxiliaryfunctions.save_data(
            PredictedData[:nframes, :], metadata, dataname, pdindex, range(nframes), save_as_csv,
        )
    return DLCscorer
...
def analyze_time_lapse_frames(
        config, directory, frametype=".png", shuffle=1, trainingsetindex=0, gputouse=None, save_as_csv=False,
        modelprefix="",
):
    """
...
def convert_detections2tracklets(
        config, videos, videotype="", shuffle=1, trainingsetindex=0, overwrite=False, destfolder=None,
        ignore_bodyparts=None, inferencecfg=None, modelprefix="", greedy=False, calibrate=False,
        window_size=0, identity_only=False, track_method="",
):
    """
